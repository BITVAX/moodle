...
EOF;
    ...
    public static function get_from_meetingid(string $meetingid): ?self {
        ...
        $existinginstanceid = $DB->get_fieldset('bigbluebuttonbn', 'id', ['meetingid' => $matches['meetingid']]);
        ...
        if (count($existinginstanceid) > 1) {
            debugging("The meeting id with ID ($meetingid) was found in multiple instances in the bigbluebuttonbn table", DEBUG_DEVELOPER);
            // return the instance that matches the $matches['instanceid']
            $existinginstanceid = array_values(array_filter($existinginstanceid, function ($id) use ($matches) {
                return $id == $matches['instanceid'];
            }));
            if (empty($existinginstanceid)) {
                debugging("The Instance ID ({$matches['instanceid']}) was not found in the bigbluebuttonbn table", DEBUG_DEVELOPER);
                return null;
            }
        }
        $instance = self::get_from_instanceid($existinginstanceid[0]);
